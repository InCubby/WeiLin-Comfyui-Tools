<template>
  <div class="weilin_prompt_ui_prompt-box">
    ...
    <div
      ...
      class="weilin_prompt_ui_panel-shell weilin_prompt_ui_panel-shell--label"
    >
      <MainLabelManager
    ...
    <!-- 主要内容容器 -->
    <div
      class="weilin_prompt_ui_panel-shell weilin_prompt_ui_panel-shell--main"
...
<script setup>
  ...
  const selectAutocomplete = (index, event) => {
    ...
    // 向后查找单词边界
    while (false && replaceEnd < currentText.length && !/[,\s]/.test(currentText[replaceEnd])) {
      replaceEnd++
    }

    // 执行替换 - 始终在tag后添加逗号和空格，光标在分隔符后
    let newText =
      currentText.substring(0, replaceStart) + tagText + ', ' + currentText.substring(replaceEnd)
